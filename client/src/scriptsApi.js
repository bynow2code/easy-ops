...
export const scriptsApi = {
  ...
  save: (script) =>
    request('/scripts', { method: 'POST', body: JSON.stringify(script) }),
  // 删除一条脚本：DELETE /api/scripts -> { ok, id }
  remove: (id) =>
    request('/scripts', { method: 'DELETE', body: JSON.stringify({ id }) }),
  // 新增分组：POST /api/groups -> { ok, groups }
  addGroup: (name) =>
    request('/groups', { method: 'POST', body: JSON.stringify({ name }) }),
  // 移除分组（级联删除其下脚本）：DELETE /api/groups -> { ok, scripts, groups }
  removeGroup: (name) =>
    request('/groups', { method: 'DELETE', body: JSON.stringify({ name }) }),
};
