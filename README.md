...
| 🔍 **搜索** | 按名称或脚本内容实时过滤 |
| 🖥️ **真实交互终端** | 基于 node-pty + xterm.js，不是「输出面板」—— 脚本能读 stdin，你也可以直接在里面敲命令；**多个终端以瀑布流卡片同屏铺开**（列数随窗口宽度自适应），每张卡片自带标题、运行状态、最大化/还原与关闭；另有「关闭全部」；终端标题为脚本名（同名脚本自动加序号区分） |
...
**界面布局**：窗口横向对半分 —— **左半**是脚本区（上 60% 脚本列表、下 40% 脚本详情预览），**右半**是终端区。

**终端瀑布流**：所有运行中的终端会以卡片形式同屏铺开，卡片高 340px，列数由窗口宽度自动决定（最小列宽 360px）—— 窗口宽就多排几列，窄了就退化成单列、纵向滚动。
