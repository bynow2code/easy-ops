...
| 🌗 **主题切换** | 工具栏（App Info 左侧）一键循环切换 跟随系统 / 浅色 / 深色，默认跟随系统，选择会被记住 |
...
1. 点击 **新建脚本**（或类似入口）。
2. 给脚本起个名字，例如「启动后端服务」。
3. 选择分组（Backend / Frontend）。如需让该脚本使用特定的 Shell 解释器（而不是全局配置），可在 **Shell Interpreter** 下拉里挑选；留空即跟随全局 Shell 配置。
...
- **导入（↑）**：选择本地 `.json` 文件，将其中脚本列表**全量覆盖**当前配置。文件需符合格式：
  ...
  - 每条脚本可含可选 `shellId`（字符串）：单独指定的 Shell 解释器 id；为空或省略表示跟随全局配置。
