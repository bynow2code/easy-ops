...
> ⚠️ **本计划的「架构」段已过时（2026-09-24 晚修订）。**
> 计划原定用受控 `Dropdown trigger={[]}` + 自挂 `onContextMenu`（方案 B），实测发现该做法丢失 antd 的
> `alignPoint` 能力，导致**菜单不跟随鼠标**；根因与推翻过程见规格文档 §3.3 与 §7.6。
> **最终实现改用 `trigger={['contextMenu']}`（方案 A），与脚本行同构；`menuOpenId` state 已删除。**
> 下文各任务步骤保留为历史记录，**以规格文档为准**。

**目标：** 让侧栏分组行（分组头）支持右键弹出菜单，行为与脚本行的整行右键一致（含**菜单出现在鼠标位置**），且右键不改变分组的展开/收起状态。
